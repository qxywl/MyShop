/*
 * 文 件 名:  VirtualProductManager.java
 * 版   权: Copyright www.teshehui.com Corporation 2015 版权所有
 * 描     述:  <描述>
 * 修 改 人:  mingbao
 * 修改时间:  2015-10-01
 * 跟踪单号: <跟踪单号>
 * 修改单号: <修改单号>
 * 修改内容: <修改内容>
 */
package com.zhaidou.product.manager;

import com.zhaidou.framework.model.RequestObject;
import com.zhaidou.framework.model.ResponseObject;

/**
 * <一句话功能简述>
 * <功能详细描述>
 * 
 * @author  mingbao
 * @version  [版本号, 2015-10-01]
 * @see  [相关类/方法]
 * @since  [产品/模块版本]
 */
public interface VirtualProductManager{
    
	/**
     * 通过RequestObject.requestParams=list[productCode]获取商品list
     * @param  requestObj RequestObject.requestParams
     * @return  ResponseObject.data=list[VirtualProduct]
     * */
    public ResponseObject getVirtualProductByCodes(RequestObject requestObj);

}
