...
<%@ attribute name="page" type="com.zhaidou.framework.util.page.Page" required="true"%>
...
<c:if test="${onchange == null}" >
    <c:if test="${target == 'dialog'}" >
    <c:set var="onchange" value="dialogPageBreak({numPerPage:this.value}) "/>
    </c:if>
    <c:if test="${target == 'navTab'}" >
